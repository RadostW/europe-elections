...
files_to_parse = (
    (here / "../data/germany/harmonised/kreisen")
    .resolve()
    .glob("germany__bundestag*.csv")
)

crosswalk_file = here / "../data/germany/harmonised/kreisen/germany__crosswalks.csv"
...
# ========
# Manual entries for some special codes
# ========
manual_crosswalks = [
    {
        **x,
        "code_latest": x.get("code_latest",x["code_start"]),
        "name_latest": x["name_start"],
        "proportion_population": 1.0,
        "proportion_area": 1.0,
        "name_chain": "Special",
    }
    for x in [
        {"code_start": "A_13999", "year_start": 1998, "name_start": "Briefwahlen"},
        {"code_start": "A_12999", "year_start": 1998, "name_start": "Briefwahlen"},
        {"code_start": "A_11100", "code_latest": "A_11000", "year_start": 1998, "name_start": "Berlin"},
        {"code_start": "A_11200", "code_latest": "A_11000", "year_start": 1998, "name_start": "Berlin"},
        {"code_start": "A_11100", "code_latest": "A_11000", "year_start": 2017, "name_start": "Berlin"},
        {"code_start": "A_11200", "code_latest": "A_11000", "year_start": 2017, "name_start": "Berlin"},
        {"code_start": "A_11100", "code_latest": "A_11000", "year_start": 2021, "name_start": "Berlin"},
        {"code_start": "A_11200", "code_latest": "A_11000", "year_start": 2021, "name_start": "Berlin"},
    ]
]

df_cross = pd.concat([df_cross, pd.DataFrame(manual_crosswalks)], ignore_index=True)


all_dfs = []

for file_path in sorted(files_to_parse):
    if "long" in file_path.name:
        continue  # germany__bundestag_long.csv
    year = re.search(r"__(\d{4})_", file_path.name).group(1)
    date = re.search(r"__(\d{4}_\d{2}_\d{2})", file_path.name).group(1)
    crosswalk_year = min([int(year), 2022])  # no croswalks available for 2022 onwards
    ...
    cw_prev = df_cross.query(f"year_start == {int(crosswalk_year)-1}")[
        ["code_start", "code_latest", "proportion_population", "name_latest"]
    ]
    cw_curr = df_cross.query(f"year_start == {int(crosswalk_year)}")[
        ["code_start", "code_latest", "proportion_population", "name_latest"]
    ]
    ...
    cw_used = pd.concat(
        [cw_used, cw_curr[cw_curr["code_start"].isin(codes_curr)]], ignore_index=True
    )
    ...
    df_expanded[scale_cols] = (
        df_expanded[scale_cols].apply(pd.to_numeric, errors="coerce").fillna(0)
    )
    df_expanded["proportion_population"] = pd.to_numeric(
        df_expanded["proportion_population"], errors="coerce"
    ).fillna(0)
    ...
    df_harmonised = df_expanded.groupby(
        ["code_latest", "name_latest"], as_index=False
    ).agg(agg_dict)

    # --- Rename columns
    df_harmonised = df_harmonised.rename(
        columns={"code_latest": "harmonised_code", "name_latest": "harmonised_name"}
    )
    ...
    party_cols = [
        c
        for c in df_harmonised.columns
        if c
        not in ["harmonised_code", "harmonised_name"]
        + eligible_voter_cols
        + issued_ballot_cols
    ]
    ...
    party_cols_sorted = (
        df_harmonised[party_cols].sum().sort_values(ascending=False).index.tolist()
    )
...
output_path = here / "../data/germany/harmonised/kreisen/germany__bundestag_long.csv"
